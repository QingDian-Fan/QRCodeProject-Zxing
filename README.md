...
- ScanQRCodeFragment为扫描二维码的fragment 只包含一个Camera的相机View 不包含任何UI，实现UI在父Activity中实现
- QRCodeUtils 为二维码工具类 ，包含解析二维码、生成二维码、控制手电筒的方法
- NativeZxing 为Jni方法用于调用zxing-cpp进行二维码操作
#### 使用
##### 配置仓库地址
```
...
- 识别二维码
```
 QRCodeUtils.analyzeBitmap(mBitmap,object : AnalyzeCallback{
            override fun onAnalyzeFailed() {
                //todo 识别失败
            }
